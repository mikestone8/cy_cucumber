...
noAccessTokenF() {
      cy.request({
       method: 'POST',
       url: 'https://api.test.retail.sse.co.uk/esbApi/onboardingApi/V1/api/onboarding', failOnStatusCode: false,
       headers: {'Content-type': 'application/json', 'Authorization': 'Bearer ' + accessTokenF}, 
       body: {   pravdaId: "TILIPRVD4531",
                 instructionType: "cos",
                 partnerBranchId: "partner_migration_id",
                 partnerTopLevelName: "Top Level Partner",
                 ...
                 expect(response.body).to.eq("UnAuthorized to access the resource.")
               })
    }

checkStatusResponseP() {
      expect(window.response).to.have.property('status', 200)
      }

FcheckStatusResponse () {
